...
public class GetTermByIdHandler : IRequestHandler<GetTermByIdQuery, Result<TermDTO>>
{
    ...
    {
        var term = await _repositoryWrapper.TermRepository.GetFirstOrDefaultAsync(f => f.Id == request.Id, null);

        if (term is null)
        {
            string errorMsg = Errors_Common.NotFoundById.FormatWith("term", request.Id);
