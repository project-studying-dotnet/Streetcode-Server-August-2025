...
public class GetTagByTitleHandlerTests
{
    ...
    {
        // Arrange
        var request = new GetTagByTitleQuery("ExistingTag");
    ...
    {
        // Arrange
        var request = new GetTagByTitleQuery("NonExistentTag");

        _mockTagRepository.Setup(r => r.GetFirstOrDefaultAsync(It.IsAny<Expression<Func<DAL.Entities.AdditionalContent.Tag, bool>>>(), null))
            .ReturnsAsync((DAL.Entities.AdditionalContent.Tag)null);

        string expectedErrorMsg = Errors_AdditionalContent.Tag_NotFoundByTitle.FormatWith(request.Title);
